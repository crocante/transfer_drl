...
from dpagent.agent_adapt.sampler import collect_samples, collect_samples_comp1, add_noise, get_model_samples, get_samples_with_buf
...
if __name__ == '__main__':
    ...
    save_dir = '../data/run_0702/adapt/0710'
    ...
    with tf.Session(config=config) as sess:
        ...
        while (counter_agg_iters < num_aggregation_iters+1):
            ...
            for k in range(inner_aggregation_iters_k):
                get_model_samples(env, local_steps_per_epoch, sess, agent, model_logger, max_ep_len)
                pi_loss, v_loss = agent.update_inner_policy(train_pi_iters, train_v_iters, target_kl,
                                                            model_logger)  # , logger
            ...
            avg_ret, avg_rew = get_samples_with_buf(env, local_steps_per_epoch, sess, agent, model_logger, max_ep_len)
            pi_loss, v_loss = agent.update_inner_policy(train_pi_iters, train_v_iters, target_kl,
                                                        model_logger)
            print("return_before_train:",avg_ret)
            print("avg_before_train:", avg_rew)
            ...
            for rollout_num in range(num_trajectories_for_aggregation):
                ...
                # x, x_m, z, z_m, selected_u, ep_rew, ep_rew_model, ret, ret_model, resulting_x = \
                #                collect_samples_comp1(1, env, steps_per_rollout_train, sess,
                #                                     agent, logger, max_ep_len)
                x, z, selected_u, ep_rew, ret, resulting_x = collect_samples_comp1(1, env, steps_per_rollout_train,
                                                                                   sess, agent, logger, max_ep_len)
                ...
                list_rewards.append(ep_rew[0:len(ep_rew)+1])
                x_list.append(x[0:len(x)+1])
                z_list.append(z[0:len(z)+1])
                # x_model_list.append(x_m[0:len(x_m)+1])
                # z_model_list.append(z_m[0:len(z_m)+1])
                # rewards_list.append(ep_rew[0:len(ep_rew)+1])
                # rewards_model_list.append(ep_rew_model[0:len(ep_rew_model)+1])
        ...
        np.savetxt(save_dir + '/training_data/training_loss.txt', training_loss_list)
        np.savetxt(save_dir + '/training_data/validation_errors_1.txt', errors_1_per_agg)
        np.savetxt(save_dir + '/training_data/validation_errors_5.txt', errors_5_per_agg)
        np.savetxt(save_dir + '/training_data/validation_errors_10.txt', errors_10_per_agg)