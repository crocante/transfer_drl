...
def mlp(x, hidden_sizes=(32,), activation=tf.tanh, output_activation=None):
    ...
    for h in hidden_sizes[:-1]:
        x = tf.layers.dense(x, units=h, activation=activation, name='hidden_'+str(l))
        l += 1
    return tf.layers.dense(x, units=hidden_sizes[-1], activation=output_activation, name='output')
...
def mlp_gaussian_policy(x, a, pi_network_params, log_std_network_params, hidden_sizes, hidden_nonlinearity, output_nonlinearity, action_space):
    ...
    _, mu = forward_mlp(output_dim=act_dim,
                        ...
                        input_var=x,
                        mlp_params=pi_network_params,
                        )
    # log_std = tf.get_variable(name='log_std', initializer=-0.5*np.ones(act_dim, dtype=np.float32))
    log_std = log_std_network_params[0]
    std = tf.exp(log_std)
...
Actor-Critics
"""
def mlp_actor_critic(x, a, pi_network_params_, v_network_params, hidden_nonlinearity=tf.tanh,
                     output_nonlinearity=None, policy=None, action_space=None):
    hidden_sizes = (64, 64)
    log_std_network_params = []
    pi_network_params = OrderedDict()
    for name, param in pi_network_params_.items():
        if 'log_std' in name:
